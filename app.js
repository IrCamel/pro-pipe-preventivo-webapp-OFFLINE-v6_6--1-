(function () {
  ...
  // Distances loader + migration (supports legacy structures and normalizes to {km,min})
  const loadDistanze = () => {
    try {
      ...
      Object.keys(d).forEach(k => {
        const v = d[k];
        // numeric legacy value -> treat as km
        if (typeof v === "number") {
          out[k] = { km: Number(v) || 0, min: 0 };
          if (v !== out[k].km) changed = true;
          return;
        }

        // object: try several possible key names (robusta migrazione)
        if (v && typeof v === "object") {
          const kmCandidates = ["km", "kmAR", "km_a_r", "distanceKm", "distanza", "km_val"];
          const minCandidates = ["min", "minuti", "tempoMinAR", "tempo", "tempo_min", "timeMin"];

          let km = null, min = null;
          for (const key of kmCandidates) {
            if (v[key] !== undefined) { km = Number(v[key]) || 0; break; }
          }
          for (const key of minCandidates) {
            if (v[key] !== undefined) { min = Number(v[key]) || 0; break; }
          }

          // fallback to properties named like numbers inside object
          if (km === null && v.km !== undefined) km = Number(v.km) || 0;
          if (min === null && v.min !== undefined) min = Number(v.min) || 0;
          ...
          // detect if original structure differs (so we can re-save normalized)
          if (v.km !== out[k].km || v.min !== out[k].min || v.kmAR !== undefined || v.tempoMinAR !== undefined) {
            changed = true;
          }
          return;
        }
      ...
      if (changed) {
        // persist normalized shape so next loads are clean
        saveDistanze(out);
      }
  ...
  function renderDistanze() {
    ...
    // keys ordinate alfabeticamente
    const keys = Object.keys(rows).sort((a, b) => a.localeCompare(b, "it", { sensitivity: "base" }));
    ...
    keys.forEach((loc) => {
      ...
      kmInput.type = "number"; kmInput.min = "0";
      kmInput.value = (d && d.km !== undefined) ? d.km : 0;
      kmInput.addEventListener("input", () => {
        const all = loadDistanze();
        all[loc] = all[loc] || { km: 0, min: 0 };
        all[loc].km = Number(kmInput.value) || 0;
        saveDistanze(all);
        computePreventivo();
        // non necessario chiamare refreshLocalitaDropdown qui, salvo che cambi il nome della località
      });
      tdKm.appendChild(kmInput); tr.appendChild(tdKm);

      const tdMin = document.createElement("td");
      const minInput = document.createElement("input");
      minInput.type = "number"; minInput.min = "0";
      minInput.value = (d && d.min !== undefined) ? d.min : 0;
      minInput.addEventListener("input", () => {
        const all = loadDistanze();
        all[loc] = all[loc] || { km: 0, min: 0 };
        all[loc].min = Number(minInput.value) || 0;
        saveDistanze(all);
        computePreventivo();
      });
      ...
      del.textContent = "🗑️"; del.className = "apply-btn";
      del.addEventListener("click", () => {
        if (!confirm(`Vuoi eliminare la località "${loc}"?`)) return;
        const all = loadDistanze();
        delete all[loc];
        saveDistanze(all);
        renderDistanze();
        refreshLocalitaDropdown();
        computePreventivo();
      });
  ...
  // Gestione tabella distanze (update + elimina)
  document.querySelector("#tabDistanze tbody").addEventListener("input", e => {
    if (e.target.matches("input[data-field]")) {
      const field = e.target.dataset.field;
      const loc = e.target.dataset.loc;
      const distanze = loadDistanze();
      if (distanze[loc]) {
        distanze[loc][field] = parseFloat(e.target.value) || 0;
        saveDistanze(distanze);
      }
    }
  });

  document.querySelector("#tabDistanze tbody").addEventListener("click", e => {
    if (e.target.classList.contains("delete")) {
      const loc = e.target.dataset.loc;
      if (confirm(`Vuoi eliminare la località "${loc}"?`)) {
        const distanze = loadDistanze();
        delete distanze[loc];
        saveDistanze(distanze);
        renderDistanze();
        refreshLocalitaDropdown(); // aggiorna subito anche il menu nel preventivo
      }
    }
  });
  ...
  addDistanza.addEventListener("click", () => {
    ...
    let newLoc = prompt("Inserisci il nome della località:");
    if (!newLoc) return;
    newLoc = newLoc.trim();
    if (!newLoc) return;

    if (current[newLoc]) {
      alert("La località esiste già.");
      return;
    }

    const km = Number(prompt("Inserisci km A/R (numero):") || 0);
    const min = Number(prompt("Inserisci minuti A/R (numero):") || 0);

    // aggiungi senza sovrascrivere le altre chiavi
    current[newLoc] = { km: Number(km) || 0, min: Number(min) || 0 };
    saveDistanze(current);
  ...
  function refreshLocalitaDropdown() {
    const distanze = loadDistanze();
    const allLoc = Array.from(new Set([...LOCALITA, ...Object.keys(distanze)]));

    // Ordine alfabetico (italiano)
    allLoc.sort((a, b) => a.localeCompare(b, "it", { sensitivity: "base" }));

    const prev = localitaSel.value || "";
    localitaSel.innerHTML = "";
    allLoc.forEach(c => {
      const o = document.createElement("option");
      o.value = c;
      o.textContent = c;
      localitaSel.appendChild(o);
    });

    // ripristina selezione se ancora presente, altrimenti imposta la prima voce se esiste
    if (prev && Array.from(localitaSel.options).some(o => o.value === prev)) {
      localitaSel.value = prev;
    } else {
      if (allLoc.length) localitaSel.value = allLoc[0];
    }
  }
